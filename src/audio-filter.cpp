...
static void run_whisper_inference(struct cleanstream_data *gf, const float *pcm32f_data,
                                  size_t pcm32f_size)
{
  info("%s: processing %d samples, %.3f sec), %d threads",
        __func__, int(pcm32f_size), float(pcm32f_size)/WHISPER_SAMPLE_RATE,
...
static struct obs_audio_data *cleanstream_filter_audio(void *data, struct obs_audio_data *audio)
{
  ...
  audio_resampler_resample(gf->resampler, (uint8_t **)output, &out_frames, &ts_offset,
                           (const uint8_t **)adata, audio->frames);

  info("%d channels, %d frames, %f ms", (int)channels, (int)out_frames, (float)out_frames / WHISPER_SAMPLE_RATE * 1000.0f);

  // run the inference
  run_whisper_inference(gf, output[0], out_frames);

  // // resample back to original sample rate
  // audio_resampler_resample(gf->resampler_back, (uint8_t **)output, &out_frames, &ts_offset,
  //                          (const uint8_t **)gf->segment_buffers, WHISPER_FRAME_SIZE);
