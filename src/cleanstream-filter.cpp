			   ...
			   uint64_t n_samples_window)
{
	...
	for (uint64_t j = 0; j < n_samples_window; j++) {
		energy_in_window =
			std::max(energy_in_window, fabsf(pcmf32[window_i + j]));
	}
			    ...
			    uint32_t sample_rate, float thold, bool verbose)
{
	...
	float first_window_energy =
		avg_energy_in_window(pcmf32, 0, n_samples_window);
	float last_window_energy = avg_energy_in_window(
		pcmf32, pcm32f_size - n_samples_window, n_samples_window);
	float max_energy_in_middle = max_energy_in_window(
		pcmf32, n_samples_window, pcm32f_size - n_samples_window);

	if (verbose) {
		blog(LOG_INFO,
		     "%s: first_window_energy: %f, last_window_energy: %f, max_energy_in_middle: %f",
		     __func__, first_window_energy, last_window_energy,
		     max_energy_in_middle);
		// print avg energy in all windows in sample
		for (uint64_t i = 0; i < pcm32f_size - n_samples_window;
		     i += n_samples_window) {
			blog(LOG_INFO,
			     "%s: avg energy_in_window %" PRIu64 ": %f",
			     __func__, i,
	...
	if (first_window_energy < max_energy_thold &&
	    last_window_energy < max_energy_thold) {
		if (verbose) {
			blog(LOG_INFO,
			     "%s: word boundary found between %" PRIu64
			     " and %" PRIu64,
			  ...
			  size_t pcm32f_size)
{
	do_log(gf->log_level, "%s: processing %d samples, %.3f sec, %d threads",
	       ...
	       float(pcm32f_size) / WHISPER_SAMPLE_RATE,
	...
	try {
		...
		error("Whisper exception: %s. Filter restart is required",
		      e.what());
	...
	if (whisper_full_result != 0) {
		...
		try {
			if (gf->detect_regex != nullptr &&
			    strlen(gf->detect_regex) > 0) {
				std::regex filler_regex(gf->detect_regex);
				if (std::regex_search(
					    text_lower, filler_regex,
					    std::regex_constants::match_any)) {
					return DETECTION_RESULT_FILLER;
				}
			}
			if (gf->beep_regex != nullptr &&
			    strlen(gf->beep_regex) > 0) {
				std::regex beep_regex(gf->beep_regex);
				if (std::regex_search(
					    text_lower, beep_regex,
					    std::regex_constants::match_any)) {
...
void process_audio_from_buffer(struct cleanstream_data *gf)
{
	...
	do_log(gf->log_level,
	       "processing %d frames (%d ms), start timestamp %" PRIu64 " ",
	...
	do_log(gf->log_level, "%d channels, %d frames, %f ms",
	...
	if (!skipped_inference) {
		...
		if (inference_result == DETECTION_RESULT_FILLER) {
			...
			if (gf->log_words) {
				info("beep segment, adding a beep %lu -> %u",
				     first_boundary, num_new_frames_from_infos);
			}
			if (gf->do_silence) {
				for (size_t c = 0; c < gf->channels; c++) {
...

	{
		...
		info_out.frames =
			num_new_frames_from_infos; // number of frames in this packet
		info_out.timestamp =
			start_timestamp; // timestamp of this packet
		circlebuf_push_back(&gf->info_out_buffer, &info_out,
				    sizeof(info_out));
		...
		while (true) {
			...
			if (input_buf_size >= segment_size) {
				do_log(gf->log_level,
				       ...
				       input_buf_size,
				       (size_t)(input_buf_size / sizeof(float)),
				       segment_size);
						...
						struct obs_audio_data *audio)
{
	...
	{
		std::lock_guard<std::mutex> lock(
			whisper_outbuf_mutex); // scoped lock
		...
		circlebuf_pop_front(&gf->info_out_buffer, &info_out,
				    sizeof(info_out));
		do_log(gf->log_level,
		       "output packet info: timestamp=%" PRIu64
		       ", frames=%" PRIu32 ", bytes=%lu, ms=%u",
		       info_out.timestamp, info_out.frames,
		       gf->output_buffers[0].size,
		...
		for (size_t i = 0; i < gf->channels; i++) {
			gf->output_audio.data[i] =
				(uint8_t *)&gf->output_data
					.array[i * info_out.frames];
			circlebuf_pop_front(&gf->output_buffers[i],
					    gf->output_audio.data[i],
...
void *cleanstream_create(obs_data_t *settings, obs_source_t *filter)
{
	...
	for (size_t c = 0; c < gf->channels;
	     c++) { // initialize the copy-output buffers
...
void cleanstream_defaults(obs_data_t *s)
{
	...
	obs_data_set_default_string(s, "detect_regex",
				    "\\b(uh+)|(um+)|(ah+)\\b");
	obs_data_set_default_string(s, "beep_regex",
...
obs_properties_t *cleanstream_properties(void *data)
{
	...
	obs_properties_add_text(ppts, "detect_regex", "detect_regex",
				OBS_TEXT_DEFAULT);
	obs_properties_add_text(ppts, "beep_regex", "beep_regex",
				OBS_TEXT_DEFAULT);
